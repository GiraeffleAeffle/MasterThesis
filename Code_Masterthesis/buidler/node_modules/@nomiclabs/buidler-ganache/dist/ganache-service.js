...
const log = debug_1.default("buidler:plugin:ganache-service");
...
class GanacheService {
        ...
        try {
            ...
            if (e.name === "TypeError") {
                if (GanacheService.error === undefined) {
                    const error = new errors_1.BuidlerPluginError(`Ganache plugin config is invalid: ${e.message}`, e);
                    log("Failed to initialize GanacheService\n", error);
                    GanacheService.error = error;
                }
            }
            else {
                throw new errors_1.BuidlerPluginError(`Failed to initialize GanacheService: ${e.message}`, e);
            }
        }
    }
    ...
    async startServer() {
        ...
        try {
            ...
            const error = new errors_1.BuidlerPluginError(`Failed to start GanacheService: ${e.message}`, e);
            if (GanacheService.error === undefined) {
    ...
    async stopServer() {
        ...
        try {
            ...
            const error = new errors_1.BuidlerPluginError(`Failed to stop GanacheService: ${e.message}`, e);
            if (GanacheService.error === undefined) {
    ...
    _validateAndTransformOptions(options) {
        ...
        if (url.hostname !== "localhost" && url.hostname !== "127.0.0.1") {
            throw new errors_1.BuidlerPluginError("Ganache network only works with localhost");
        }
        ...
        try {
            ...
            throw new errors_1.BuidlerPluginError(`Ganache network config is invalid: ${e.message}`, e);
        }
        // Test for unsupported commands
        if (options.accounts !== undefined) {
            throw new errors_1.BuidlerPluginError("Config: ganache.accounts unsupported for this network");
        }
    ...
    _checkForServiceErrors() {
        if (GanacheService.error !== undefined) {
            ...
            throw new errors_1.BuidlerPluginError(`An error occurred in GanacheService: ${GanacheService.error.message}`, GanacheService.error);
        }
    }
    _snakeCase(str) {
