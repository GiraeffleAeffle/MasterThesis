#include "/home/max/seal-test/seal-c++/SEAL/native/examples/examples.h"
...
int main() {
 ...
 struct LCI_DATA {
   string Timestamp;
   struct SUPPLY_DATA supply_data;
   struct INPUT_DATA input_data;
   struct OUTPUT_DATA output_data;
 }lci_data;


    ifstream ifs("completelcidata0.json");
    Json::Reader reader;
    Json::Value obj;
    reader.parse(ifs, obj); // reader can also read strings
    lci_data.supply_data.SupplychainID = obj["SupplychainID"].asString();
    lci_data.supply_data.FromEthAddress = obj["FromEthAddress"].asString();
    lci_data.supply_data.ToEthAddress = obj["ToEthAddress"].asString();
    lci_data.supply_data.BatchID = obj["BatchID"].asString();
    lci_data.supply_data.ProdInfoHash = obj["ProdInfoHash"].asString();
    lci_data.Timestamp = obj["Timestamp"].asString();
    const Json::Value& input_json = obj["Input"]; // array of characters
    cout << "Input: " << endl;
    for (int i = 0; i < input_json.size(); i++){
      if(i == 0)
      {
        cout << "   Energy: ";
        lci_data.input_data.energy.value = input_json[i]["Energy"].asFloat();
        cout << lci_data.input_data.energy.value;
        cout << " SI: "  << endl;
        lci_data.input_data.energy.unit = input_json[i]["SI"].asString();
        cout << lci_data.input_data.energy.unit;

      }/*
      if(i == 1)
      {
        cout << "   Water: ";
        inp_data.water.value = input_json[i]["Water"].asFloat();
        cout << inp_data.water.value;
        cout << " SI: " << endl;
        inp_data.water.unit = input_json[i]["SI"].asString();
        cout << inp_data.water.unit;
      }*/
      if(i == 1 )
      {
          cout << "   Sonst: " << ": " << endl;
          lci_data.input_data.misc.value = new float[input_json[i]["Sonst"].size()];
          lci_data.input_data.misc.unit = new string[input_json[i]["Sonst"].size()];
          for (int j = 0; j < input_json[i]["Sonst"].size(); j++){
            cout << "      Mat" << j << ": ";
            lci_data.input_data.misc.value[j] = (input_json[i]["Sonst"][j]["Mat"+to_string(j)]).asFloat();
            cout << lci_data.input_data.misc.value[j];
            cout << " SI: " ;
            lci_data.input_data.misc.unit[j] = input_json[i]["Sonst"][j]["SI"].asString();
            cout << lci_data.input_data.misc.unit[j] << endl;
          }
      }
    }

    const Json::Value& output = obj["Output"]; // array of characters
    cout << "Output: " << endl;
    for (int k = 0; k < output.size(); k++){
      if(k == 0)
      {
        cout << "   CO2: ";
        lci_data.output_data.co2.value = output[k]["CO2"].asFloat();
        cout << lci_data.output_data.co2.value;
        cout << " SI: ";
        lci_data.output_data.co2.unit =  output[k]["SI"].asString();
        cout << lci_data.output_data.co2.unit << endl;
      }
      else {
        lci_data.output_data.misc.value = new float[output[k]["Sonst"].size()];
        lci_data.output_data.misc.unit = new string[output[k]["Sonst"].size()];
        cout << "   Sonst: " << ": " << endl;
        for (int l = 0; l < output[k]["Sonst"].size(); l++){
          cout << "      Mat" << l << ": ";
          lci_data.output_data.misc.value[l] = (output[k]["Sonst"][l]["Mat"+to_string(l)]).asFloat();
          cout << lci_data.output_data.misc.value[l];
          cout << " SI: ";
          lci_data.output_data.misc.unit[l] = (output[k]["Sonst"][l]["SI"]).asString();
          cout << lci_data.output_data.misc.unit[l] << endl;
        }
      }
}
...
// 1. Gleiche Supplychain //


// 2. Matrix aufstellen, die relationen zu den verschiedenen Knoten der Lieferkette aufzeigt
...
// 3. LCI für selbe BatchID aufstellen
...
print_example_banner("Example: CKKS Basics");
    ...
    */
    ...
    /// LÖSCHEN DER DYNAMISCHEN ARRAYS ////

    delete[] lci_data.input_data.misc.value;
    lci_data.input_data.misc.value = NULL;
    delete[] lci_data.input_data.misc.unit;
    lci_data.input_data.misc.unit = NULL;
